...
class MulticlassTrainer:
    def __init__(self, config: Configuration, device: torch.device, model, output_result_path: Path,
        ...
        if self.learning_rate_decay and self.step_enable:
            ...
            self.skip_first = config.get_param_value("learning_rate_decay/skip_first", False) or False
    ...
    def init_optim(self):
        ...
        if self.learning_rate_decay and self.step_enable:
            ...
            print('LR decay will be applied at iterations: {} with a factor of {}'.format(self.steps, self.factor))

    def train(self):
        self.train_loader, self.val_loader = self.dataset.get_train_loader()
        log_model_structure(self.output_result_path / "train_model_structure", self.model)
    ...
    def train_epoch(self, epoch):
        ...
        train_loss = 0.0
        progress_msg = '  Epoch: {} - Iteration: {} - train_loss: {:.05} - train_acc:{:.05}'
        epoch_hits = 0
        images_seen = 0
...
class IncrementalMulticlassTrainer(MulticlassTrainer):
    def __init__(self, config: Configuration, device: torch.device, model, output_result_path: Path,
        ...
        self.num_groups = len(self.num_classes_per_group)
        ...
        self.first_iterations = config.get_param_value('first_iterations', False) or self.base_iterations
        self.iterations_backend = config.get_param_value('fc_architecture/iterations_backend', False)
    ...
    def train(self):
        ...
        for k_classifier in range(self.num_groups):
            ...
            if self.iterations_backend is None and k_classifier == 1:
                self.model.freeze_backend()

            self.model.change_fc(k_classifier)
            log_model_structure(self.output_result_path / "train_model_structure_{}".format(k_classifier),
                                self.model)
            self.model.train()
            self.init_optim()
            ...
            iterations = self.first_iterations if k_classifier == 0 else self.base_iterations

            self.max_iterations = iterations + k_classifier * self.per_class_iterations
            self.best_val_acc = 0

            # TODO: uncomment if variable iterations
            # if self.learning_rate_decay and self.step_enable:
    ...
    def train_epoch(self, epoch, k_classifier):
        ...
        num_classes = self.dataset.num_classes_per_group[k_classifier]
        if k_classifier > 0:
            num_classes += 1
        ...
        for i_batch, (images, labels) in enumerate(iterator):
            ...
            loss.backward()
            self.optimizer.step()
            if self.learning_rate_decay and self.step_enable and \
                    (not self.skip_first or (self.skip_first and k_classifier > 0)):
                self.scheduler.step()

            if global_iteration == self.iterations_backend and k_classifier == 0:
                self.model.freeze_backend()
...
class BinaryTrainer:
    ...
    def train(self):
        ...
        for i_class in range(self.dataset.num_classes):
            ...
            log_model_structure(self.output_result_path / "train_model_structure_{}".format(i_class), self.model)
            self.model.train()
...
class BinaryTrainerM(BinaryTrainer):
    def train(self):
        ...
        for i_class in range(1, self.dataset.num_classes):
            ...
            log_model_structure(self.output_result_path / "train_model_structure_{}".format(i_class),
                                self.model)
            self.model.train()
