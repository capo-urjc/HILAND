...
class MultiClassBaseDataset(BaseDataset, ABC):
    ...
    def get_mc_test_loader(self, k_classifier=None):
        ...
        if k_classifier is not None:
            classes = list(range(0, sum(self.num_classes_per_group[:k_classifier + 1])))
...
class NormalMCDataset(MultiClassBaseDataset):
    ...
    def get_mc_train_loader(self, k_classifier):
        ...
        c_classes = np.array(
            list(range(sum(class_intervals[:k_classifier + 1]), sum(class_intervals[:k_classifier + 2]))))
        no_c_classes = np.array(list(range(0, sum(class_intervals[:k_classifier + 1]))))
        ...
        labels_c = train_labels[idx_c] - sum(class_intervals[:k_classifier + 1])
        if k_classifier > 0:
            labels_c += 1
...
class ResamplingMCEDataset(MultiClassBaseDataset):
    def __init__(self, config: Configuration, output_result_path, device, model, dataset_class):
        ...
        for label in labels:
            label = label.item()
            self.class_indexes[int(label)] = torch.where(torch.tensor(self.targets) == label)[0]
    ...
    def get_data_subset(self, class_idx, train_data, train_labels, class_zero, k_classifier):
        if len(class_idx) != 0:
            ...
            if self.resampling_current_samples == "all" and class_zero:
                class_samples = self.resampling_samples
            elif self.resampling_current_samples == "all" and not class_zero:
                class_samples = max([torch.unique(train_labels[train_labels == i], return_counts=True)[1] for i in
                                     class_idx]).item() * len(class_idx)
            elif self.resampling_current_samples is not None and class_zero:
                class_samples = self.resampling_samples
            ...
            if class_zero:
                ...
                class_intervals = [0] + self.num_classes_per_group
                labels_no_c = train_labels[indexes] - sum(class_intervals[:k_classifier + 1])
                if k_classifier > 0:
                    labels_no_c += 1
        else:
    ...
    def get_mc_train_loader(self, k_classifier, after_last_classifier=False):
        ...
        if not after_last_classifier:
            images_c, labels_c, indexes_c = self.get_data_subset(c_classes, train_data, train_labels, False,
                                                                 k_classifier)
...
class ConvexHullDataset(BaseDataset):
    ...
    def get_train_loader(self, i_class: int = -1, model=None):
        ...
        labels_no_c_originals = torch.unique(dataset_train.targets[idx_no_c]).numpy()
