...
from torchvision.models import resnet18, vgg16, resnet50, densenet161, inception_v3, resnet34
...
class CNN(nn.Module, ABC):
    def __init__(self, in_channels: int, num_classes: int, config: Configuration, device):
        ...
        self.device = device
        self.fused_fc = None
        self.cnn, self.num_features = self.get_model()
        ...
        if self.network_type == NetworkTypes.BINARY_ENSAMBLE:
            ...
            num_groups = int(num_classes / self.num_classes_per_group)
            fcs = [self.create_fc(NetworkTypes.MULTICLASS, num_classes=self.num_classes_per_group)] + \
                  [self.create_fc(NetworkTypes.MULTICLASS, num_classes=self.num_classes_per_group + 1)
        ...
        if len(hidden_layers_size) == 0:
            if self.network_type == NetworkTypes.BINARY_ENSAMBLE or self.network_type == NetworkTypes.BINARY_ENSAMBLE_B:
                ...
                if k_classifier is not None:
                    num_classes = (k_classifier + 1) * self.num_classes_per_group
                ...
                for idx, header in enumerate(layers_to_fuse):
                    idx = torch.tensor(
                        list(range(idx * self.num_classes_per_group, (idx + 1) * self.num_classes_per_group)),
                        dtype=torch.long, device=self.device)
                    self.fused_fc[0].weight[idx, :] = torch.tensor(header[0].weight[-self.num_classes_per_group:],
                                                                   device=self.device)
                    self.fused_fc[0].bias[idx] = header[0].bias[-self.num_classes_per_group:]
                self.old_i_class = -1
                self.model = self.fused_fc
        else:
...
class VGG(CNN):
    ...
    def get_model(self):
        ...
        cnn = nn.Sequential(model.features, model.avgpool)
        return cnn, model.classifier[0].in_features
