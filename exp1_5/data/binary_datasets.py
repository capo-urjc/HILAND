...
class BaseDataset:
    def __init__(self, config: Configuration, output_result_path, device, model, dataset_class):
        ...
        self.sample_dataset = self.dataset_class(root=self.dataset_path, download=self.download, train=True)
        sample_data = torch.tensor(self.sample_dataset.data, dtype=torch.float)
        ...
        self.classes = self.sample_dataset.classes
        self.num_classes = len(self.sample_dataset.classes)
        self.num_channels = self.sample_dataset.data.shape[3] if len(self.sample_dataset.data.shape) == 4 else 1
        self.reduction = None

        current_model = model.cnn
        class_features = []
        images_dataset = self.dataset_class(root=self.dataset_path, download=self.download, train=True,
                                            transform=self.test_data_transform)

        features_data_loader = DataLoader(images_dataset, batch_size=self.train_batch_size, drop_last=False,
                                          shuffle=False)
        ...
        num_elem = torch.tensor(images_dataset.data).shape[0]
        class_features = torch.vstack(class_features).view(num_elem, -1)
        self.features_dataset = FeaturesDataset(class_features, torch.tensor(images_dataset.targets))
    ...
    def gen_train_val_loader(self, images_no_c, images_c, labels_no_c, labels_c, labels_no_c_org, labels_c_org):
        ...
        dataset_train = FeaturesDataset(train_new_images, train_new_labels)
    ...
    def __str__(self):
        ...
        info += '  Classes: {}\n'.format(self.sample_dataset.classes)
...
class NormalDataset(BaseDataset):
    ...
    def get_train_loader(self, i_class: int = -1, model=None):
        ...
        if self.network_type == NetworkTypes.BINARY_ENSAMBLE or self.network_type == NetworkTypes.BINARY_ENSAMBLE_M or \
            ...
            images = dataset_train.features[idx_c]
            ...
            return self.gen_train_val_loader(torch.tensor([]), dataset_train.features, torch.tensor([]),
                                             dataset_train.labels, torch.tensor([]), dataset_train.labels)
...
class ConvexHullDataset(BaseDataset):
    ...
    def get_train_loader(self, i_class: int = -1, model=None):
        ...
        images_no_c = dataset_train.features[idx_no_c]
        labels_no_c_org = dataset_train.labels[idx_no_c]
